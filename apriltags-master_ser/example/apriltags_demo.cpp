...
public:
  ...
  void print_detection(AprilTags::TagDetection& detection) const {
    ...
    //cout << m_tagSize << endl << m_fx << endl << m_fy << endl << m_px << endl << m_py<<endl;
    //cout << endl << endl;
    //cout << translation.size() << endl << rotation.size() << endl;
    //cv::waitKey(10000);
    ...
    if (/*recv(uart_rtcm.fd,R_buf,3)*/1)
    {

        S_buf[0] = 0xaa;
        S_buf[1] = (int)(translation(1)*1000)>>8;
        S_buf[2] = (int)(translation(1)*1000);
        S_buf[3] = (int)(translation(0)*1000)>>8;
        S_buf[4] = (int)(translation(0)*1000);
        //S_buf[5] = int(translation(2)*100)/256;
        //S_buf[6] = int(translation(2)*100)%256;
        //S_buf[7] = check1;
        S_buf[5] = (int)(yaw/PI*180*10)>>8;
        S_buf[6] = (int)(yaw/PI*180*10);
        //S_buf[9] = check2;
        //S_buf[10] = int(pitch/PI*180);
        //S_buf[11] = check3;
        //S_buf[12] = int(roll/PI*180);
        S_buf[7] = 0xbb;
        for(i = 0;i < 8;i++)
        {
          len = UART0_Send(fd,S_buf,8);
          if(len > 0)
